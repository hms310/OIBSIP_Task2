...
class Maindemo extends JFrame
{
    ...
    {
    	int bestScore=100;
        int remainingGuesses = 10; // set the number of remaining guesses

        public void actionPerformed(ActionEvent e) {
            int a = Integer.parseInt(guess.getText());
            count = count + 1;
            remainingGuesses = remainingGuesses - 1; // decrement the remaining guesses

            if (remainingGuesses < 0) { // if there are no more guesses left
                trylabel.setText("Sorry, you have no more guesses left!");
                guess.setEditable(false);
            } else { // if there are remaining guesses
                if (a < rand) {
